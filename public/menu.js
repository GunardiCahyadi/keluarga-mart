...
function renderCard(menu){
	...
	itemCardQuantityEditorButtonMinus.classList.add('item-card-quantity-editor-button')
	itemCardQuantityEditorButtonMinus.innerText = '-'
	//function kurang
	itemCardQuantityEditorButtonMinus.addEventListener('click',function(){
		itemCardQuantityEditorQuantity.innerText = String(Number(itemCardQuantityEditorQuantity.innerText) - 1)
		removeItemFromCart(menu.id,cart)
		console.log(cart)
		if (itemCardQuantityEditorQuantity.innerText === '0') {
			itemCardBodyFooter.replaceChild(itemCardAddToCart,itemCardQuantityEditor)
		}
	})
	
	const itemCardQuantityEditorQuantity = document.createElement('span')
	itemCardQuantityEditorQuantity.classList.add('item-card-quantity-editor-quantity')
	itemCardQuantityEditorQuantity.innerText= '1'
	
	const itemCardQuantityEditorButtonPlus = document.createElement('button')
	itemCardQuantityEditorButtonPlus.classList.add('item-card-quantity-editor-button')
	...
	itemCardAddToCart.addEventListener('click',function(){
		//replaceChild
		itemCardQuantityEditorQuantity.innerText='1'
		itemCardBodyFooter.replaceChild(itemCardQuantityEditor,itemCardAddToCart)
